...
- [ ] 8.1 Start the application and verify all built-in themes render correctly in both light and dark modes
- [ ] 8.2 Create a custom user theme in `~/.config/bludio/themes/light/` and verify it appears in the dropdown
- [ ] 8.3 Select the custom user theme and verify it applies correctly
- [ ] 8.4 Delete the custom theme and verify the app falls back to the default theme
- [ ] 8.5 Verify font family switching still works after removing it from `Theme`
- [ ] 8.6 Verify settings persistence still works after all changes
- [ ] 8.7 Test with a malformed user theme file and verify it is skipped with a warning
