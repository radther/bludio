...
Allows users to add custom themes by placing JSON files in `~/.config/bludio/themes/`. Themes are loaded at startup alongside built-in themes and can override built-in themes with the same ID.
